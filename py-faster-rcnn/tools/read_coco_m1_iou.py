...
imdb = get_imdb('coco_2014_val')

path = '../output/faster_rcnn_end2end/coco_2014_val/'
...
with open('%s/vgg16_context_concat_4fc_img224_faster_rcnn_iter_496698/detections_faster.pkl'%(path), 'rb') as f:
	data_faster = pickle.load(f)

with open('%s/vgg16_context_concat_4fc_img224_faster_rcnn_iter_496698/detections_context.pkl'%(path), 'rb') as k:
	data_context = pickle.load(k)

gt = imdb.get_annotation()

with open('gt_m1.csv', 'w') as csvfile:
	...
	for i in xrange(imdb.num_images):
		...
		image_id = imdb.image_path_at(i).split('/')[-1].split('.jpg')[0].split('COCO_val2014_')[1].lstrip('0')
